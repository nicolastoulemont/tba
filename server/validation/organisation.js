...
const validateOrgInput = async data => {
  ...
  if (!Validator.isLength(data.description, { min: 5, max: 2000 })) {
    let descriptionLengthError = {
      path: 'description',
      message:
        'The organisation description must be between 5 and 2000 characters'
    };
  ...
  if (!Validator.isLength(data.address, { min: 1, max: 150 })) {
    let addressLengthError = {
      path: 'address',
      message:
        'The organisation must have an address between 1 and 150 characters'
    };
  ...
  if (!Validator.isLength(data.registryId, { min: 1, max: 50 })) {
    let registryIdLengthError = {
      path: 'registryId',
      message:
        'The EU registry ID  must have a type between 1 and 50 characters'
    };
